...
def calcular_grau_distancia_final(df):
    """
    Versão final que calcula o grau de distância de cada comentário em relação ao comentário original da thread.
    Lida com threads desconectadas, tratando cada componente conectado como uma sub-thread separada.
    ...
    for target_id in df_copy['target_id'].unique():
        ...
        print(f"\nProcessando Thread {target_id}: {len(thread_df)} comentários")
        ...
        print(f"  Componentes conectados encontrados: {len(componentes)}")
        
        # Processar cada componente separadamente
        for i, componente in enumerate(componentes):
            print(f"  Processando Componente {i+1}: {len(componente)} nós")
            
            # Filtrar dados do componente atual
            componente_ids = list(componente)
            componente_df = thread_df[thread_df['id'].isin(componente_ids)].copy()
            
            # Criar grafo do componente
            G_comp = G.subgraph(componente_ids)
            
            # Identificar comentários originais neste componente
            comentarios_originais = []
            
            # Critério 1: parent_label como "Alvo da Conversa"
            alvo_conversa = componente_df[componente_df['parent_label'] == 'Alvo da Conversa']['id'].tolist()
            comentarios_originais.extend(alvo_conversa)
            
            # Critério 2: parent_name como 'Raiz'
            raiz = componente_df[componente_df['parent_name'] == 'Raiz']['id'].tolist()
            comentarios_originais.extend(raiz)
            
            # Critério 3: parent_id é nulo (primeiro nível)
            sem_parent = componente_df[componente_df['parent_id'].isnull()]['id'].tolist()
            comentarios_originais.extend(sem_parent)
            
            # Critério 4: Nós sem entrada no grafo (raízes topológicas)
            raizes_topologicas = [n for n in G_comp.nodes() if G_comp.in_degree(n) == 0]
            comentarios_originais.extend(raizes_topologicas)
            
            # Critério 5: Comentários com parent_id que não está no componente (comentários originais externos)
            parent_ids_externos = []
            for _, row in componente_df.iterrows():
                if pd.notnull(row['parent_id']) and row['parent_id'] not in componente_df['id'].values:
            ...
            print(f"    Comentários originais identificados: {len(comentarios_originais)}")
            if len(comentarios_originais) > 0:
                print(f"    IDs: {comentarios_originais[:3]}...")  # Mostrar primeiros 3
            
            # Calcular distância para cada comentário do componente
            for _, row in componente_df.iterrows():
                ...
                for comentario_original in comentarios_originais:
                    try:
                        # Se o comentário original está no componente atual
                        if comentario_original in G_comp.nodes():
                            distancia = nx.shortest_path_length(G_comp, comentario_original, comentario_id)
                            ...
                            if comentario_original in parent_ids_externos:
                                ...
                                filhos_diretos = componente_df[componente_df['parent_id'] == comentario_original]['id'].tolist()
                                for filho in filhos_diretos:
                                    if filho in G_comp.nodes():
                                        try:
                                            distancia = nx.shortest_path_length(G_comp, filho, comentario_id) + 1
                                            min_distancia = min(min_distancia, distancia)
...
def main():
    """
    Função principal para executar o cálculo de grau de distância final
    """
    ...
    print("\nCalculando graus de distância (versão final)...")
    df_com_grau = calcular_grau_distancia_final(df)
    
    print("\nAnálise dos resultados:")
    analisar_distribuicao_graus(df_com_grau)
    
    # Salvar resultado
    output_file = 'Dados/conjuntoDeDados_com_grau_distancia_final.tsv'
